...
const getItem = async (recordResult, user, Qs) => {
  return new Promise(async (resolve) => {
    ...
    const line = recordResult;
    mylog(line);
    const recordId = recordResult.record_id;
    ...
    const [userResult] = await pool.query(Qs.searchUserQs, [createdBy]);
    if (userResult.length === 1) {
      createdByName = userResult[0].name;
    }
    
    const [groupResult] = await pool.query(Qs.searchGroupQs, [applicationGroup]);
    if (groupResult.length === 1) {
      applicationGroupName = groupResult[0].name;
    }
 
    const [itemResult] = await pool.query(Qs.searchThumbQs, [recordId]);
    if (itemResult.length === 1) {
      thumbNailItemId = itemResult[0].item_id;
    }

    const [countResult] = await pool.query(Qs.countQs, [recordId]);
    if (countResult.length === 1) {
      commentCount = countResult[0]['count(*)'];
    }

    const [lastResult] = await pool.query(Qs.searchLastQs, [user.user_id, recordId]);
...
const tomeActive = async (req, res) => {
  ...
  let searchRecordQs =
    'select record_id, title, application_group, created_by, created_at, updated_at from record where status = "open" and (category_id, application_group) in (';
  let recordCountQs =
  ...
  let count = 0;
  const Qs = {
    searchUserQs : 'select name from user where user_id = ?',
    searchGroupQs : 'select name from group_info where group_id = ?',
    searchThumbQs : 'select item_id from record_item_file where linked_record_id = ? order by item_id asc limit 1',
    countQs : 'select count(*) from record_comment where linked_record_id = ?',
    searchLastQs : 'select access_time from record_last_access where user_id = ? and record_id = ?',
  };
  const items = await Promise.all(recordResult.map( (record) => {
    return getItem(record, user, Qs);
  }));
...
const allClosed = async (req, res) => {
  ...
  const searchRecordQs = `select record_id, title, application_group, created_by, created_at, updated_at from record where status = "closed" order by updated_at desc, record_id asc limit ? offset ?`;
...
const mineActive = async (req, res) => {
  ...
  const searchRecordQs = `select record_id, title, application_group, created_by, created_at, updated_at from record where created_by = ? and status = "open" order by updated_at desc, record_id asc limit ? offset ?`;
